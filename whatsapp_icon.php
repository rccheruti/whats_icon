<?php
/*
Plugin Name: WhatsIcon
Description: Icone para disparar mensagem do whatsapp.
Version: 1.0
Author: Roger Cheruti
*/

if (!defined('ABSPATH')) {
    die;
}

require_once plugin_dir_path(__FILE__) . 'includes/whats_icon_settings.php';
require_once plugin_dir_path(__FILE__) . 'includes/whats_icon_scripts.php';
include_once plugin_dir_path(__FILE__) . 'includes/whats_icon_upgrade_handler.php';